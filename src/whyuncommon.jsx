...
class WHYUncommon extends Component {
  ...
  async componentDidMount() {
    await axios
      .get(
        polygonscanapi +
          `?module=stats&action=tokensupply&contractaddress=${NFTCONTRACT2}&apikey=${polygonscanapikey}`
      )
      .then((outputa) => {
        this.setState({
    ...
    await axios
      .get(
        moralisapi + `/nft/${NFTCONTRACT2}/owners?chain=mumbai&format=decimal`,
        { headers: config }
      )
      .then((outputb) => {
  ...
  render() {
    ...
    async function enable() {
      contract.methods
        .setApprovalForAll(STAKINGCONTRACT2, true)
        .send({ from: account });
    }
    ...
    async function mint0() {
      ...
      await Web3Alc.eth.getMaxPriorityFeePerGas().then((tip) => {
        Web3Alc.eth.getBlock("pending").then((block) => {
          ...
          var maxFee = maxPriority + baseFee;
          currency.methods
            .approve(NFTCONTRACT2, String(totalAmount))
            .send({
              from: account,
            })
            .then(
              currency.methods.transfer(NFTCONTRACT2, String(totalAmount)).send(
                {
                  from: account,
                  maxFeePerGas: maxFee,
                  maxPriorityFeePerGas: maxPriority,
